...
def main() -> int:
    ...
    parser.add_argument("--user", default=os.environ.get("GITHUB_USER", "zeiddata-dev"))
    ...
    org_repos, org_warnings = fetch_public_repos(args.org, "orgs", token, args.offline)
    user_repos, user_warnings = fetch_public_repos(args.user, "users", token, args.offline)
    merged: dict[str, dict[str, Any]] = {}
    for item in org_repos + user_repos:
        key = item.get("full_name") or item.get("name")
        if key:
            merged[key] = item
    repos = sorted(merged.values(), key=lambda i: str(i.get("updated_at") or ""), reverse=True)[:8]
    warnings = org_warnings + user_warnings
    existing_radar = ""
