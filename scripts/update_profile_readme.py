...
def require_expected_repo() -> None:
    ...
    if "github.com/Zeid-Data/.github" not in url and "github.com:Zeid-Data/.github" not in url:
        print(f"[FAIL] wrong repo remote: {url}")
...
def table(headers: list[str], rows: list[list[Any]]) -> str:
    ...
    out = []
    out.append("| " + " | ".join(clean_cell(h) for h in headers) + " |")
...
def repo_rows(repos: list[dict[str, Any]], warnings: list[str]) -> list[list[Any]]:
    ...
    for repo in repos:
        ...
        if isinstance(url, str) and url.startswith("https://"):
            name_cell = f"[{clean_cell(name)}]({url})"
        else:
            name_cell = clean_cell(name)
...
def build_readme(org: str, now: dict[str, Any], defensive: dict[str, Any], repos: list[dict[str, Any]], warnings: list[str]) -> str:
...
{table(["Adversary Behavior", "Telemetry", "Defensive Control", "Zeid Data Build"], map_rows)}
...
def main() -> int:
    ...
    repos, warnings = fetch_org_repos(args.org, token, args.offline)

    content = build_readme(args.org, now, defensive, repos, warnings)
